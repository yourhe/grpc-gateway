...
import (
	...
	"github.com/yourhe/grpc-gateway/protoc-gen-react-redux/httprule"
)
...
func Test_binding_PathTmplToJSTmpl(t *testing.T) {
	type fields struct {
	...
	tests := []struct {
		...
		// TODO: Add test cases.
		{
			...
			fields: fields{
				&descriptor.Binding{
					PathTmpl: httprule.Template{Template: "/v1/test/{name}"},
				},
			},
			want: "/v1/test/:name",
		},
		{
			name: "test muilt change",
			fields: fields{
				&descriptor.Binding{
					PathTmpl: httprule.Template{Template: "/v1/{action}/{name}"},
				},
			},
			want: "/v1/:action/:name",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := b.PathTmplToJSTmpl(); got != tt.want {
				t.Errorf("binding.PathTmplToJSTmpl() = %v, want %v", got, tt.want)
			}
		})
	}
}
